...
trait Huffman extends HuffmanInterface {
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case Nil => Nil
    case head :: tl => (Leaf(head._1, head._2) :: makeOrderedLeafList(tl)).sortWith(weight(_) < weight(_))
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case f :: s :: tl => (makeCodeTree(f, s) :: tl).sortWith(weight(_) < weight(_))
    case _ => trees
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encode_aux(tree: CodeTree)(chr: Char): List[Bit] = tree match {
      case Fork(left, right, ch, weight) => if (chars(left).contains(chr)) 0 :: encode_aux(left)(chr) else 1 :: encode_aux(right)(chr)
      case Leaf(char, weight) => Nil
    }
    text.flatMap(encode_aux(tree))
  }
